...
#else
#define __WDATE__ __DATE__
#endif

ILstring _ilVendor		= IL_TEXT("Abysmal Software");
ILstring _ilVersion		= IL_TEXT("Developer's Image Library (DevIL) 1.6.8pre " __WDATE__);
...
ILvoid ilDefaultStates()
{
	...
	ilStates[ilCurrentPos].ilDoNotDecompressDxtc = IL_FALSE;
	...
	switch (Mode) {
		...
		case IL_KEEP_DXTC_DATA:
			*Param = ilStates[ilCurrentPos].ilKeepDxtcData;
			break;
		case IL_DO_NOT_DECOMPRESS_DXTC:
			*Param = ilStates[ilCurrentPos].ilDoNotDecompressDxtc;
			break;
		case IL_ORIGIN_MODE:
...
ILvoid ILAPIENTRY ilSetInteger(ILenum Mode, ILint Param)
{
	switch (Mode)
	{
		...
		case IL_DO_NOT_DECOMPRESS_DXTC:
			if (Param == IL_FALSE || Param == IL_TRUE) {
				ilStates[ilCurrentPos].ilDoNotDecompressDxtc = Param;
				return;
			}
			break;
		case IL_MAX_QUANT_INDEXS:
