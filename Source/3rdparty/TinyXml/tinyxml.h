...
public:
	TiXmlBase()	:	userData(0), _refs(1) {}
	virtual ~TiXmlBase()			{}

	// Release and AddRef for SmartPointer
	void Release() 
	{
		if (_refs == 1)
		{
...
//		return ( f.buffer && SaveFile( f.buffer ));
	...
	/*
	Saves the XML into stream.
	Stream must support Write(const void*, int) method.
	*/
	template<class T>
	bool SaveStream(T* stream) const
	{
		TiXmlPrinter printer;
		Accept(&printer);
	...
	/*
	Loads XML from Stream.
	Stream must support int Read(void*, int) method.
	If length parameter is 0, the stream also must support GetLength() and Seek() methods.
	*/
	template<class T>
	bool LoadStream (T* stream, long length, TiXmlEncoding encoding = TIXML_DEFAULT_ENCODING)
	{
